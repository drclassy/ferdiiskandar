    ...
    <>
      ...
      >
        <span className="abby-toggle-avatar">
          <Image src="/assets/abby/abby-avatar.png" alt="Abby" width={36} height={36} className="abby-toggle-img" />
        </span>
        {!open && <span className="abby-toggle-label">Tanya Abby</span>}
        {open && <span className="abby-toggle-close" aria-hidden="true">✕</span>}
      </button>
        ...
        >
          <header className="abby-header">
            <div className="abby-header-identity">
              <Image src="/assets/abby/abby-avatar.png" alt="Abby" width={36} height={36} className="abby-header-avatar" />
              <div>
            ...
            <div className="abby-header-actions">
              {phase === 'chat' && (
                <button type="button" className="abby-back-btn" onClick={handleBack} aria-label="Kembali ke pilihan mode">
                  ← Mode
                </button>
              )}
              <button type="button" className="abby-close-btn" onClick={() => setOpen(false)} aria-label="Tutup Abby">
                ✕
              </button>
            </div>
          </header>
          ...
          {phase === 'welcome' && (
            <div className="abby-welcome">
              <div className="abby-welcome-image">
                <Image src="/assets/abby/abby-main.png" alt="Abby, asisten AI dr Ferdi Iskandar" width={160} height={220} className="abby-main-img" />
              </div>
              ...
              <div className="abby-modes">
                {abbyConfig.visitor_modes.map((mode) => (
                  <button key={mode.id} type="button" className="abby-mode-btn" onClick={() => handleModeSelect(mode)}>
                    {mode.label}
                  </button>
                ))}
              </div>
            ...
            <>
              <div className="abby-body" aria-live="polite">
                {messages.length === 0 && (
                  <div className="abby-suggestions">
                    ...
                    {visitorMode.suggested_questions.map((q, i) => (
                      <button key={i} type="button" className="abby-suggestion-btn" onClick={() => sendMessage(q)} disabled={loading}>
                        {q}
                      </button>
                    ))}
                  </div>
                ...
                {loading && (
                  <div className="abby-bubble abby-bubble-assistant">
                    <p className="abby-typing-dots"><span /><span /><span /></p>
                  </div>
                )}

                {error && <div className="abby-error"><p>{error}</p></div>}
              ...
              <div className="abby-cta-row" aria-label="Tautan cepat">
                {FEATURED_CTAS.map((cta) => (
                  <Link key={cta.intent} href={cta.href} className="abby-cta-btn">{cta.label}</Link>
                ))}
              </div>
              ...
              <form className="abby-form" onSubmit={handleSubmit}>
                <input
                  ...
                  maxLength={2000}
                />
                <button type="submit" className="abby-send" disabled={loading || input.trim().length === 0} aria-label="Kirim pesan">
