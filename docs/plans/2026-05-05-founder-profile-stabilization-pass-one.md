...
describe('Contact', () => {
  it('never renders dead placeholder links', () => {
    ...
    expect(screen.queryByRole('link', { name: /contact sentra healthcare ai/i })).not.toBeInTheDocument()
  ...
  return (
    <section className="fi-contact fi-section" id="contact">
      <div>
        ...
        <p>
          Untuk kolaborasi strategis, pengembangan produk healthcare AI, atau diskusi
          tentang clinical intelligence infrastructure, gunakan jalur koordinasi resmi
          yang ditampilkan pada permukaan ini.
        </p>
      </div>
...
describe('HomePage', () => {
  it('renders a main landmark with the founder sections', () => {
    ...
    expect(screen.getByRole('heading', { name: /human care, augmented by intelligence/i })).toBeInTheDocument()
  })
})
```
