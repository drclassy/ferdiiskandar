...
| Basic rate limiting | `createFixedWindowRateLimiter`, 20 req / 60s, per-process in-memory | API routes | Bypass risk if proxy headers spoofed; not shared across instances |
